                    ...
                    <ul>
                        <xsl:for-each select="document($toc)/toc/item">
                            ...
                            <xsl:if test="not(not(@subkey = ''))">
                                <li><a href="{@key}.htm"><xsl:value-of select="@name"/></a></li>
                            </xsl:if>
                        </xsl:for-each>
                    </ul>
                </div>
            ...
            <ul>
                <li>
                    <xsl:if test="not($parents[1]/@subkey = '')">
                        <a class="{$parentCurrent}" href="{$parents[1]/@key}-{$parents[1]/@subkey}.htm"><xsl:value-of select="$parents[1]/@name"/></a>
                    </xsl:if>
                    <xsl:if test="not(not($parents[1]/@subkey = ''))">
                        <a class="{$parentCurrent}" href="{$parents[1]/@key}.htm"><xsl:value-of select="$parents[1]/@name"/></a>
                    </xsl:if>
                    <xsl:call-template name="chain">
                    ...
                    <xsl:if test="count($parents)=1">
                        <ul>
                            <xsl:for-each select="$childs">
                                ...
                                <xsl:if test="not(@subkey = '')">
                                    <li><a class="{$current}" href="{@key}-{@subkey}.htm"><xsl:value-of select="@name"/></a></li>
                                </xsl:if>
                                <xsl:if test="not(not(@subkey = ''))">
                                    <li><a class="{$current}" href="{@key}.htm"><xsl:value-of select="@name"/></a></li>
                                </xsl:if>
                            </xsl:for-each>
                        </ul>
                    </xsl:if>
