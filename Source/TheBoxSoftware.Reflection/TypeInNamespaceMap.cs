...
namespace TheBoxSoftware.Reflection {
	...
	internal class TypeInNamespaceMap {
		private Dictionary<string, List<Entry>> typeInNamespace = new Dictionary<string, List<Entry>>();
		...
		public void Add(string inNamespace, Core.COFF.MetadataTables table, Core.COFF.MetadataRow row) {
			if (!typeInNamespace.ContainsKey(inNamespace)) {
				typeInNamespace.Add(inNamespace, new List<Entry>());
			}
			typeInNamespace[inNamespace].Add(new Entry(table, row.FileOffset));
		}
		...
		public List<string> GetAllNamespaces() {
			return this.typeInNamespace.Keys.ToList<string>();
		}
		...
		public TypeDef FindTypeInNamespace(string inNamespace, string typeName) {
			...
			List<Entry> entries = typeInNamespace[inNamespace];
			for (int i = 0; i < entries.Count; i++) {
				Entry currentEntry = entries[i];
				TypeDef def = assembly.File.Map.GetDefinition(currentEntry.Table, currentEntry.FileOffset) as TypeDef;
		...
		public bool ContainsNamespace(string theNamespace) {
			return this.typeInNamespace.ContainsKey(theNamespace);
		}
